...
\def\idxref#1{%
   ...
   \indexwrite{#1}{0}\ignorespaces
   \endgroup
}


% For creating an index entry for control space.
\def\blankidxref{\syidxref{\ }}
...
%
\def\ttidxref#1{\indexwrite{#1}{1}\ignorespaces}

% \ccidxref generates an index entry for a double-caret character.
%
\def\ccidxref#1{\indexwrite{#1}{2}\ignorespaces}

% \chidxref generates an index entry for a special characters.
%
\def\chidxref#1{\indexwrite{#1}{3}\ignorespaces}

% \syidxref generates an index entry for a control symbol
%
\def\syidxref#1{\indexwrite{#1}{4}\ignorespaces}
...
%
\def\ctsidxref #1{\indexwrite{#1}{5}\ignorespaces}

% \aridxref generates an index entry for an argument type
%
\def\aridxref#1{\indexwrite{#1}{6}\ignorespaces}

% \fnidxref generates an index entry for a filename extension
%
\def\fnidxref#1{\indexwrite{#1}{7}\ignorespaces}

% \mcidxref generates an index entry for some miscellanous macro
%
\def\mcidxref#1{\indexwrite{#1}{8}\ignorespaces}

% \eridxref generates an index entry for some error messages
%
\def\eridxref#1{\indexwrite{#1}{9}\ignorespaces}

% \directidx write an index entry with special characters of makeindex
%
\def\directidx#1{\expandafter\sidx\expandafter{#1}\ignorespaces}
...
% Split subentries from entries and save them in macros
%
\newif\ifsub \newtoks\mainentry \newtoks\subentry
...
\def\@split#1//#2\@splitMark{%
  \ifempty{#2}%
     \ifsub
        \subentry={#1}%
     \else
        \mainentry={#1}\subentry={}%
     \fi
  \else
     \mainentry={#1}\subtrue
     \@split#2\@splitMark
...
% #1 is the text of the entry, #2 is the type of the entry
% type 0 for general entries     <- \idxref
% type 1 for special terms       <- \ttidxref
% type 2 for caret notations     <- \ccidxref
% type 3 for special characters  <- \chidxref
% type 4 for control symbols     <- \syidxref
% type 5 for control words       <- \ctsidxref
% type 6 for kind of arguments   <- \aridxref
% type 7 for filename extensions <- \fnidxref
% type 8 for miscellanous macros <- \mcidxref
% type 9 for error messages      <- \eridxref
%
\def\indexwrite#1#2{%
   \splitentry{#1}%
   \ifcase #2 % 0
      ...
      \toks2 = {}%
   \or % 1
      \toks0 = \expandafter{\the\mainentry @\tt}%
      \toks2 = {\rm}%
   \or % 2
      \toks0 = \expandafter{\expandafter\^\expandafter
                            \^\the\mainentry @\tt\char`\^\char`\^}%
      \toks2 = {\rm}%
   \or % 3
      \toks0 = \expandafter{\the\mainentry @\tt\char`}%
      \toks2 = {\rm}%
   \or % 4
      \toks0 = \expandafter{\the\mainentry+@\hskip -.5em\tt\\\char`}%
      \toks2 = {\rm}%
   \or % 5
      \toks0 = \expandafter{\the\mainentry @\hskip -.5em\tt\\}%
      \toks2 = {\rm}%
   \or % 6
      \toks0 = \expandafter{\the\mainentry @\hskip -.5em$\langle$}%
      \toks2 = {$\rangle$}%
   \or % 7
      \toks0 = \expandafter{\the\mainentry @\hskip -.5em\tt.}%
      \toks2 = {\rm\ 文件}%
   \or % 8
      \toks0 = \expandafter{\the\mainentry @\csname }%
      \toks2 = {\endcsname}%
   \or % 9
      \toks0 = \expandafter{\the\mainentry @\hskip -.5em\tt<}%
      \toks2 = {>\rm}%
   \fi
   %
   \edef\option{\ifbix begin\else\ifeix end\fi\fi}%
   \edef\option{\option\ifpix\ifx\option\empty\else,\fi pagemarkup=textit\fi}%
   \ifx\option\empty\else\edef\option{[\option]}\fi
   \toks4 = \expandafter{\option}%
   %
   \edef\temp{\the\subentry}%
   \edef\next{\noexpand\sidx\the\toks4{%
                 \the\toks0\noexpand\the\mainentry\the\toks2%
                 \ifx\temp\empty\else!\the\subentry\fi}}%
   \next
...
\def\concept#1 \par{%
   ...
   \conceptindexfalse
   % sometimes we need to index a concept in another way
   \ifx\conceptindexsort\relax \else
      \global\indextoks = \expandafter{\expandafter
                          \mcidxref\expandafter{\conceptindexsort}}%
      \global\let\conceptindexsort\relax
   \fi
...
% \ctscaret precede a character #1 with double carets and generates an index entry
% for it.  #1 is blank-delimited. \ctscaret does not generate a
% cross-reference definition.
%
\def\ctscaret#1 {%
   \ctsbasic{\char`\^\char`\^#1}{}%
   \pix\ccidxref{#1}}

% \ctsymbol prints an control symbol #1 and generates an index entry
% for it.  #1 is blank-delimited. \ctssymbol does not generate a
% cross-reference definition.
%
\def\ctsymbol#1 {%
   \ctsbasic{\\\char`#1}{}%
...
% \ctsymbolx is like \ctsymbol, but leaves more space after its argument.
%
\def\ctsymbolx#1 {%
   \ctsbasic{\\\char`#1}{}\quad
   \pix\syidxref{#1}}

% \ctsymboln is like \ctsymbol, but doesn't generates an index entry for it.
%
\def\ctsymboln#1 {%
   \ctsbasic{\\\char`#1}{}}

% \ctsymbolxn = \ctsymbolx + \ctsymboln
%
\def\ctsymbolxn#1 {%
   \ctsbasic{\\\char`#1}{}\quad}
...
% \ctsydisplay displays a control symbol.
% #1 is the control symbol, #2 is the cross-reference label,
% blank-delimited.
%
\def\ctsydisplay#1 #2 {%
   \basicdisplay{$#1$}{\\\char`#1}%
   \ifxref \ctsxrdef{#2}\else \xreftrue \fi
   \pix\syidxref{#1}%
}

% \ctsydisplayn is like \ctsydisplay, but doesn't generates an index entry for it.
%
\def\ctsydisplayn#1 #2 {%
   \basicdisplay{$#1$}{\\\char`#1}%
   \ifxref \ctsxrdef{#2}\else \xreftrue \fi
}

% \actdisplay displays an special character.
% #1 is the control symbol corresponding to the character,
% #2 is the cross-reference label, blank-delimited.
%
\def\actdisplay#1 #2 {%
   \basicdisplay{$\char`#1$}{\char`#1}%
...
\def\capac{\cstwofalse\capoutch}
\def\capactwo{\cstwotrue\capoutch}
\def\capsy{\cstwofalse\capoutsy}
\def\capsyidx{\cstwofalse\capoutsyidx}
\def\capcs{\cstwofalse\capoutcs}
\def\capcstwo{\cstwotrue\capoutcs}

% Parameters of \capoutxx macros
...
% Special characters
%
\def\capoutch#1#2#3#4{%
  \caplineout{\enspace\char`#1}{#2}{#3}{#4}%
  \chidxref{#1}%
}

% Control symbols
%
\def\capoutsy#1#2#3#4{%
  \caplineout{\\\char`#1}{#2}{#3}{#4}%
  \syidxref{#1}%
}
\def\capoutsyidx#1#2#3#4#5{%
  \caplineout{\\\char`#1}{#2}{#3}{#4}%
  \directidx{#5+@\hskip-.5em\tt\\\char`#5\rm}%
}
...
\def\capoutcs#1 #2#3#4{%
   \begingroup \catcode `\@ = \letter
   \ifempty {#4}\temptrue\else\tempfalse\fi
   % \iftemp needed to avoid \if ... \else ... \fi nesting problems
   \toks0 = {\caplineout{\\#1}{#2}{#3}}%
   \iftemp
      \toks2 = \expandafter{\csname #1\endcsname}%
   \else
      \toks2 = {#4}%
   \fi
   \edef\nextaction{\the\toks0 {\the\toks2}}%
   \nextaction
   \ctsidxref {#1}%
   \endgroup
}
...
\def\TeXyhz{\TeX{} 用户组织} % 以得到正确的索引排序
...
\def\xrefdelim[#1]{第~\xrefn{#1}~页} % this form doesn't need braces
