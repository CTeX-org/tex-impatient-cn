...
\xrdef{introparms}
有些命令实际上是参数（parameter，见\xref{parameter}）。
^^{参数//作为命令}
你可以按下面两种方式使用这种参数：
\olist
\li 将该参数的值用作其他命令的参量。
例如 \hbox{|\vskip\parskip|} 这个命令生成大小%
等于\zglue 参数|\parskip|（段落间距）的垂直间距。
\li 通过赋值修改该参数的值。
...
只是它们需要额外的参量指定表格项。
例如，|\catcode| 是类别码表格的名称（\xref{category code}）。
...
另外有些空格则变成宽度可变的间隔即``\zglue'' (\xref{glue})；
\zglue 有其自然宽度（它希望的宽度），但也可以伸展或者收缩。
\TeX\ 排版要求右页边对齐（通常如此）的段落时，
将调整各行的\zglue 的宽度以让各行正好在页边结束。%
...
插入单词间空隔，结束一个段落，等等。
从单个排版字符及其他简单排印元素开始，
\TeX\ 将^{盒子}一层层放入其他盒子中\seeconcept{box}，
这一整套盒子构造了一个页面^^{页面}。
...
它总处于六种\emph{模式}^^{模式}（\xref{mode}）的其中一种模式中。
它所组装的东西的类型定义了它所在的模式。
首先有两种普通模式：普通水平模式用于组装段落（在它们分为多行之前），
普通垂直模式用于组装页面。
其次有两种限制模式：限制水平模式用于水平地添加元素形成水平盒子，
内部垂直模式用于垂直地添加元素以形成竖直盒子（非页面盒子）。
最后还有两种数学模式：行内数学模式用于组装段落内部的数学公式，
