...
在引用（如``Fig.~8''）、姓名（如``V.~I\null. Lenin''）
以及三个点$\ldots$中，最好把单词绑定在一起，
这样 \TeX\ 不会在它们的内部断行（三个点表示省略号）。
...
~^^{字体} ^^{字符//特殊字符} ^^{重音}
~^^{音符} ^^{花色}
~\chidxref{\$} \chidxref{\&} \chidxref{\#} \chidxref{\_} \chidxref{\%}
...
~%spacing look right.
这是意大利体单词{\it italic words}，粗体单词{\bf bold words}，
而这里是意大利体、粗体和罗马体这几种字体混用的效果：
{\it put {\bf bold} and {\rm roman} inside italic}。
当倾斜字体后面紧跟非倾斜字体时，我们在其间插入一个``倾斜校正''
({\tt \\/})，使得中间的空白看起来较舒适。
...
你可以在 !xrefdelim[accents] 和 !xrefdelim[fornlets]
找到 \TeX\ 的 欧洲语音和字母表。
...
某些讨论稿需要这样做，使得议员们能够在其上作一些标记。书商们
...
~\idxref{Palatino 字体}
~\idxref{Zapf, Hermann}
~\idxref{计算机现代字体} \mcidxref{Metafont}
