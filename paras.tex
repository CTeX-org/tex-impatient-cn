...
這些命令可以產生各種歐文字符和連寫.
它們在文本中遇到偶而出現的歐文單詞和詞組時顯得很有用.
但是如果需要排印大量的歐文文本時, 
你可能會去使用一個為這些語言定製的 \TeX\ 版本.
\footnote{\TeX\ 用戶組織 (\xref{resources}) 可為你提供 \TeX\ 的欧洲语
  言版本.}
...
^^{special characters}
%
\easy\ctspecialx # \ctsxrdef{@pound} {英磅符號 \# \footnote{zpxing: 位
...
\ctsx dag {短剑符號 \dag}
\ctsx ddag {雙剑符號 \ddag}
\ctsx copyright {版權符號 \copyright}
\ctsx P {段落符號 \P}
\ctsx S {章節符號 \S}
\explain
這些命令可以產生各種特殊字符和符號.
前五個是必需的, 因為 \TeX\ 默認地把符號 (|#|, |$|, |%|, |&|, |_|) 作特殊的用途.
你不用為了使用這些符號而進入\minref{數學模式}.
...
必须指出|\par|不会让\TeXi{}开始新的一段；它仅仅告诉\TeX{}去结束一段。
...
要想减小段落间|\par|产生的空白，通常可以使用命令|\vskip -\lastskip|。当你需要写一个\miniref{宏}而让它与前面是不是空行无关的时候，这个命令就有用。
...
通常，你应该在一个\miniref{群组}里为|\leftskip|\minref{赋值}，来避免影响其余的文档部分。可是在一个段落内的群组里改变|\leftskip|的值是无意义的---只有段落\emph{末尾}处|\leftskip|的值才会被\TeX{}用来拆段成行。\minrefs{line
break}
...
\cts ttraggedright {}
\explain
这些命令让\TeX{}以“^{左对齐}”的方式排版文档，其中单词间距保持正常，即间距一样，不需伸缩。因此右边距是不均匀的。而\TeX默认的则是均匀对齐^^{justification}，即左右边距一样。均匀对齐的文本单词间距将会有伸缩以产生均匀的右边距。有些排版者偏好使用左对齐。因为左对齐避免了页面上的空白参差不齐。
\minrefs{均匀对齐的文本}
