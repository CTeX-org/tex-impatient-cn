...
因此，任何在 \<replacement text> 中的内容会被展开，
但那些产生盒子或粘连的排列或命令依然保持原来的样子。
比如 \TeX\ 在处理这个定义时，在由 |\edef| 的定义中的 \<replacement text> 中的 |\hbox|
命令依然会保持命令的格式，而不会变成一个盒子。
一般情况下，并不能明显看出什么会被展开，什么不会，
但你可以在 \knuth{页~212-215} 找到一张可被展开的控制序列列表。
...
%\endexample
%\enddesc
|\write|, |\message|, |\errmessage|, |\wlog| 和 |\csname| 
命令会把它们的记号列使用和 |\edef| 替换文本相同的法则进行展开。
^^|\write//用 {\tt\\edef} 规则展开|
...
它告诉 \TeX\ 该宏的参数可以包括 |\par| 记号 (\xref{\@par}),
该记号一般表示一个段的结束。
^^|\par//在宏参量中|
如果 \TeX\ 尝试把一个没有 |\long| 的宏定义展开，
而该宏的任意一部分的参数包括了一个 |\par| 记号，
\TeX\ 就会报错错这是一个失控的参数。
...
这个命令告诉 \TeX\ 把 \<control sequence> 的定义设为 \<token$_2$>（可以由 |\let| 完成），
然后按正常的方式来处理 \<token$_1$> 和 \<token$_2$>。
在宏定义的最后 |\futurelet| 很有用，因为它可以给你提供一个
在 \TeX\ 处理未处理的记号前，查看它后面的记号的方法。
\example
\def\predict#1{\toks0={#1}\futurelet\next\printer}
...
它提供了一种把记号列合并成控制序列的方式，
包括一些你一般情况下不能直接写的形式。\<token list> 可以包含控制序列。
|\csname| 命令会把这个记号列展开，使用和 |\edef| 替换文本相同的法则（\xref\edef ）。
如果最后的展开会产生不是字符的东西，\TeX\ 会报错。
|\csname| 把一个记号列转为一个控制序列；
