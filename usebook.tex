...
\li \chapterref{hvmodes}，``水平和垂直模式命令''，
介绍水平模式（段落和水平盒子）和垂直模式（页面和垂直盒子）中对应或等同的命令。
...
某些命令需要特定类型的参数（\xref{arg1}）。
命令的参数提供 \TeX{} 执行该命令时所需的额外信息。
每个参数都放在尖括号中，并用意大利体标明它的类型：
