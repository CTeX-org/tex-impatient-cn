...
只要字符到达 \TeX\ 的^{食道}\seeconcept{\texpouxi}并被解释完，它的类别码就不再改变。
用 ^|\char| 命令\ctsref{\char}生成的字符没有类别码，
因为 |\char| 命令是一个让 \TeX\ 生成特定\refterm{字体}中的某个特定字符的指令。
例如，`|\|'（通常的转义字符）的 \ascii\ ^^{ASCII} 码是 $92$。
...
13&活动字符 & |~| 和 |^^L| \tequiv\ascii\ \asciichar{form\ feed}
   ^^{活动字符} ^^{换页符} \chidxref{\~} \ccidxref{L} \cr
14&注释符   & |%| ^^{注释}
   \chidxref{\%//其类别码}
   \cr
...
在用它的眼睛读取字符时\seeconcept{\texpouxi}，\TeX\ 执行一些“过滤”，
...
\ccidxref{M}\ccidxref{J}\ccidxref{I}
...
到达\TeX\ 的胃\seeconcept{\texpouxi}的每个\refterm{记号}都作为一个命令，
...
\>\uparrow \>\downarrow \>\updownarrow
\>\Uparrow \>\Downarrow \>\Updownarrow$}
\chidxref{\(} \chidxref{\)} \chidxref{\[} \chidxref{\]}
\chidxref{\/} \syidxref{\{} \syidxref{\}}
...
dd& \didotpt\mcidxref{didotpt} & ^{迪多点}  & 1157 {\didotpt}s = 1238 points\cr
...
\TeX\ 在四种不同的背景中用到文件：
\olist
\li “^{源码文件}”是 \TeX\ “眼睛”读取\seeconcept{\texpouxi}，并根据它的一般法则解释的文件。
...
例如，|cmbx12| 字体的形状文件 可以是压缩格式的 |cmbx12.pk|\fnidxref{pk}，
也可以是一般格式的 |cmbx12.gf|\fnidxref{gf}。
...
只要 \TeX\ 位于水平模式中，它的胃\seeconcept{\texpouxi}就在构造项目%
...
\ulist
\li 你可以用 |\-| \syidxref{\-} 命令\ctsref{\@minus} 插入自定连字符，
以允许 \TeX\ 在特定位置连字化。
^^{自定连字符}
...
是在该字体的度量文件（\tfmfile\fnidxref{tfm}）中指定的。
\margin{paragraph deleted to save space; most of the material was
already in this section.}
...
\TeX\ 在食道中展开它所遇到的宏\seeconcept{\texpouxi}，
...
\TeX\ 在胃中处理你的输入时\seeconcept{\texpouxi}，
...
要么是一个\refterm{控制序列}。
\TeX\ 用它的眼睛从文件中读取字符串\seeconcept{\texpouxi}，
并用它的嘴将字符串咀嚼成一个个记号。
...
只要 \TeX\ 位于竖直模式中，它的胃\seeconcept{\texpouxi}就在构造项目%
...
\fnidxref{dvi//用 \b\tt\\special\e 插入的素材}
