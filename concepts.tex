...
\TeX\ 有两种数学模式：
^{行内数学模式}用于构造放在文本行中的公式，
而^{陈列数学模式}用于构造单独一行显示的公式。
包围在 |$| 里面的公式表示一个行内数学公式，
而包围在 |$$| 里面的公式表示一个陈列数学公式。
%\TeX\ will accept most \refterm{commands:command} in
%math mode. If it encounters a command in math mode that doesn't make
%sense in a formula, it will complain.
...
%fractions in text.
%\endconcept
\defterm{标号尺寸}（script size）描述字体族中三个相关\refterm{字体}的其中一个。
^^{family//script size in}
标号尺寸比\refterm{文本尺寸}小但比\refterm{小标号尺寸}大。
对上标，下标以及文本中分式的分子和分母，\TeX\ 用标号尺寸。
\endconcept
...
%\endconcept
\defterm{小标号尺寸}（scriptscript size）描述字体族中三个相关\refterm{字体}的其中一个。
^^{family//scriptscript size in}
对第二级的上标、下标、分子和分母，\TeX\ 用小标号尺寸。
