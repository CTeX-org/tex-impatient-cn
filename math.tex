...
除了这里列出的命令以外, \TeX\ 也把 `|=|' \chidxref{\=} 和``arrow'' 命令 (\xref{arrows}) 作为关系运算符.
...
%^^|\int//limits after|
积分算符 |\int| 和 |\oint| 是两个例外——除非像 |\int\limits| 这样指定，
\TeX\ 总是将极限放在算符的后边。（\plainTeX\ 将 ^|\int| 和 ^|\oint|
定义为算符后跟着 |\nolimits| 的宏——这就是其默认表现有所不同的原因。）%
^^|\int//极限放在后面|
...
\cts displaylimits {}
\explain
此命令让 \TeX\ 在陈列样式中将极限放在算符（包括积分符）的上边和下边，
在文内样式中将极限放在算符的后边。
...
\begindesc
\bix^^{间隔//数学公式中的间隔}
\ctsymboln \! \ctsxrdef{@shriek} \pix\directidx{"\"!+@\hskip-.5em\tt\\\char`"\"!\rm}
\ctsymbol \, \ctsxrdef{@comma}
\ctsymbol \> \ctsxrdef{@greater}
\ctsymbol \; \ctsxrdef{@semi}
\explain
